...
.config(["$routeProvider", function($routeProvider){
	$routeProvider.when('/quiz/:activityId', {
		templateUrl : '/ng_asparticipant/views/start_activity.html',
		controller:'StartActivityController',
		requireLogin: 1,
		title: "Quiz",
	})
	.when('/activity_alert', {
		templateUrl : '/ng_asparticipant/views/activity_alert.html',
	...
	.when('/activity/:activityId/question/:questionId', {
		templateUrl: '/ng_asparticipant/views/question_result.html',
...
.controller('StartActivityController', function($rootScope, $cookieStore, $location, $route, $sce, $routeParams, $scope, $window, $locale, $timeout, ActivityService, AlertService, Socket, ngDialog){
	...
	$scope.paginate = function(id){
		$rootScope.isLoading = true;
		ActivityService.FindByID(participantId, id).success(function(res, status){
	...
	$scope.submit = function(form, data){
		...
		if(form.$valid){
			...
			if(data.answers.length > 0){
				...
				$rootScope.isLoading = true;
				ActivityService.postByID(participantId, $scope.activityId, data).success(function(res, status){
					$rootScope.isLoading = false;
					if($scope.activity.type == 'polls'){
						...
						if ($scope.activity.feedback){
							...
							if(res.point == 0) ngDialog.open({ scope: $scope, template: '/ng_student/views/feedback.html', className: 'ngdialog-theme-plain', });
							else ngDialog.open({ scope: $scope, template: '/ng_student/views/feedback.html', className: 'ngdialog-theme-plain', });
						}else{
						...
						if ($scope.activity.feedback){
							...
							if($scope.completed) $location.path('activity_alert');
							else $route.reload();
						}
					}
				}).error(function(error, status){
	...
	$scope.prevnext = function(id){
		$scope.answerError.$error = {};
		$rootScope.isLoading = true;
		ActivityService.FindByQID(participantId, $scope.activityId, id).success(function(res, status){
			$timeout.cancel($scope.mytimeout);
			if(res.question == null){
				AlertService.success(res.message);
				$location.path('/');
...
.controller('QuestionResultController', function($rootScope, $cookieStore, $route, $routeParams, $scope, $location, ActivityService, AlertService, Socket){
	...
	$scope.getQuestion = function(id, qid){
		$rootScope.isLoading = true;
		ActivityService.questionResult(participantId, id, qid).success(function(res, status){
			$scope.count = res.count;
			$scope.question = res.question;
			$scope.notAnswerd = res.notAnswerd;
			for (var i = 0; i < $scope.question.options.length; i++){
